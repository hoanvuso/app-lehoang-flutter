...
import 'package:flutter_lehoang_extracare/resources/component/common/pageturn/src/builders/widget.dart';

class CustomPageTurn extends StatefulWidget {
  CustomPageTurn({
    Key? key,
  ...
  @override
  CustomPageTurnState createState() => CustomPageTurnState();
}

class CustomPageTurnState extends State<CustomPageTurn> with TickerProviderStateMixin {
  int pageNumber = 0;
  List<Widget> pages = [];
  ...
  @override
  void didUpdateWidget(CustomPageTurn oldWidget) {
  ...
  Future _onDragFinish() async {
    if (_isForward != null) {
      if (_isForward == true) {
        if (!_isLastPage && _controllers[pageNumber].value <= (widget.cutoff + 0.3)) {
          await nextPage();
        } else {
        ...
        if (!_isFirstPage && _controllers[pageNumber - 1].value >= widget.cutoff - 0.3) {
          await previousPage();
        } else {
          if (_isFirstPage) {
            await _controllers[pageNumber].forward();

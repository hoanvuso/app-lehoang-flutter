...
final forgotPassChangeViewModel = ChangeNotifierProvider.autoDispose((ref) =>
    ForgotPassChangeViewModel(ref.read(authRepositoryProvider), ref.read(prefsProvider), ref.read(broadCastProvider)));

class ForgotPassChangeViewModel extends BaseLoadingChangeNotifier {
  final AuthRepository _authRepository;
  final PrefsProvider _prefsProvider;
  final AppBroadCast _broadCastProvider;

  ForgotPassChangeViewModel(this._authRepository, this._prefsProvider, this._broadCastProvider);

  String? username;
  String? otp;

  Future<User?> changePassword(String newPass, String reNewPass) async {
    final request = RequestChangeForgotPass(
      username: username,
      password: newPass,
    ...
    final user = await handleResultAPI<User?>(
      _authRepository.changeForgotPass(request),
      isShowLoading: true,
    );
